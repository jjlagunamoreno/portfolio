...
const linkClass =
  'inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl border border-zinc-800 text-sm sm:text-base font-semibold text-zinc-100 no-underline transition hover:-translate-y-0.5 hover:border-purple-500 hover:text-purple-400'
...
function App() {
  ...
  return (
    <main className="min-h-dvh flex flex-col items-center justify-center gap-6 p-6 text-zinc-100">
      ...
      <section className="mt-14 sm:mt-6 w-full max-w-lg rounded-2xl border border-zinc-800 bg-zinc-900 p-6 sm:p-10 text-center shadow-2xl">
        ...
        <h1 className="whitespace-nowrap text-[clamp(1rem,5.2vw,1.5rem)] font-bold">
          {t.name}
        </h1>
        <p className="mt-1 font-semibold text-purple-400">{t.role}</p>
        <p className="mb-5 text-sm text-zinc-400">{t.location}</p>
        <p className="mb-7 leading-relaxed text-zinc-400">{t.bio}</p>

        <nav className="flex flex-nowrap justify-center gap-2 sm:flex-wrap sm:gap-3" aria-label="Links">
